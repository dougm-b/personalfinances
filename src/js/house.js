...
// ══════════════════════════════════════════
// Total já pago com base nos débitos mensais configurados que já decorreram
// (por período) + eventuais pagamentos/ajustes adicionais (extraPaid).
function loanScheduledByPlan(){
  const cur = todayKey().slice(0,7);
  return (state.house.loan.paymentPlans||[]).map(p => {
    let count = 0, m = p.from;
    while (m <= p.to && m <= cur) { count++; m = nextMonth(m); }
    return { plan: p, count, total: round2(count * p.amount) };
  });
}
function loanScheduledPaid(){ return round2(loanScheduledByPlan().reduce((s,x) => s + x.total, 0)); }
function loanPaidToDate(){ return round2(loanScheduledPaid() + (state.house.loan.extraPaid || 0)); }
// Saldo em dívida = total emprestado − o que já foi pago (débitos decorridos + ajustes)
function computeLoanRemaining(){
  return Math.max(0, round2((state.house.loan.total||0) - loanPaidToDate()));
}
...
function saveHouseRemaining(){
  // correção manual: acerta o pagamento adicional para o saldo bater no valor indicado
  const desired = parseFloat(document.getElementById('house-remaining').value)||0;
  state.house.loan.extraPaid = round2((state.house.loan.total||0) - loanScheduledPaid() - desired);
...
function saveLoanPayment(){
  ...
  state.house.loan.extraPaid = round2((state.house.loan.extraPaid||0) + amt);
...
function renderHouse(){
  ...
  const estimate = computeLoanRemaining() <= 0
    ? 'Empréstimo pago na totalidade 🎉'
    : (proj && proj.months > 0
        ? `Ao ritmo do débito mensal configurado (${fmtEUR(proj.monthly)}/mês), terminas de pagar em <b style="color:var(--acc)">${fmtMonth(proj.end)}</b> — daqui a ${proj.months} ${proj.months===1?'mês':'meses'}.`
        : 'Configura um débito mensal para estimar quando terminas de pagar.');
  document.getElementById('house-loan-detail').innerHTML = `
    ...
    <div class="note-box" style="margin:10px 0">📅 ${estimate}</div>
...
function openLoanHistory(){
  ...
  // pago com base nos débitos mensais configurados que já decorreram, por período
  const byPlan = loanScheduledByPlan().filter(x => x.count > 0);
  const extra = round2(loan.extraPaid || 0);
  document.getElementById('loan-history-body').innerHTML = `
    ...
    <div class="form-label" style="margin-top:12px">Montantes pagos (débito mensal configurado que já decorreu):</div>
    ${byPlan.length ? `<div class="timeline">${byPlan.map(x => `
      <div class="tl-item"><div class="tl-dot" style="background:var(--good)"></div>
        <div class="tl-info"><div class="t">${fmtEUR(x.plan.amount)}/mês · ${fmtMonth(x.plan.from)} → ${fmtMonth(x.plan.to)}</div><div class="s">${x.count} ${x.count===1?'mês decorrido':'meses decorridos'}</div></div>
        <div class="tl-val">${fmtEUR(x.total)}</div>
      </div>`).join('')}${extra !== 0 ? `
      <div class="tl-item"><div class="tl-dot" style="background:var(--good)"></div>
        <div class="tl-info"><div class="t">Pagamentos/ajustes adicionais</div></div>
        <div class="tl-val">${fmtEUR(extra)}</div></div>` : ''}</div>` : '<div class="row-detail">Ainda sem débitos decorridos.</div>'}
